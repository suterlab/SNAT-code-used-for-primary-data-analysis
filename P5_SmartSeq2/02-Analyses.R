...
resDir <- here("Results", "SS2_P5_woFilter50k", "02-Analyses")
dir.create(resDir, recursive = TRUE)
figNumer <- 1

# Upgrade Seurat v2 object to v3 object
scData <- readRDS(here("Results", "SS2_P5_woFilter50k", "01-Seurat", "scData.rds"))
scData <- UpdateSeuratObject(scData)
scData <- RenameIdents(scData, "0"="mSC1", "1"="pmSC", "2"="iSC", "3"="prol. SC")
...
p2 <- DimPlot(scData, cols=cellCols, reduction = "tsne", label=TRUE) +
  theme(legend.position = "none")
p3 <- DimPlot(scData, reduction = "umap", label=TRUE, group.by="res.0.4") +
  theme(legend.position = "none")
p4 <- DimPlot(scData, reduction = "tsne", label=TRUE, group.by="res.0.4") +
  theme(legend.position = "none")
